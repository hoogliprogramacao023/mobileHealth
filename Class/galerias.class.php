...
if(empty($GaleriasInstanciada)) {
	...
	class Galerias {
		...
		function rsDados($id='', $orderBy='', $limite='', $destaque='', $categoria='', $tipo = '') {
			...
			if(isset($_POST['acao']) && $_POST['acao'] == 'addGaleria') {
				...
				$tipo = filter_input(INPUT_POST, 'tipo', FILTER_SANITIZE_STRING);
					...
					try{
						...
						$sql = "INSERT INTO tbl_galeria_imagem (foto, destaque, descricao_imagem, legenda_imagem, categoria, tipo) VALUES (?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
		...
		function editar($redireciona='galerias.php') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaGaleria') {
				...
				try { 
					...
					$sql = "UPDATE tbl_galeria_imagem SET foto=?, destaque=?, descricao_imagem=?, legenda_imagem=?, categoria=?, tipo=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $destaque);   
					$stm->bindValue(3, $descricao_imagem);   
