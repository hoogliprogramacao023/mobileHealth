			...
			if(isset($_POST['acao']) && $_POST['acao'] == 'addPublicacao') {
				...
				$urlAmigavel = gerarTituloSEO($titulo);
					try{
						...
						$sql = "INSERT INTO tbl_publicacoes (foto, titulo, postado_por, conteudo, data_postagem, link, breve, meta_title, meta_keywords, meta_description, foto1, url_amigavel, descricao_imagem, legenda_imagem, id_categoria) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
		...
		function editar($redireciona='publicacoes.php') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaPublicacao') {
				...
				try { 
					...
					$sql = "UPDATE tbl_publicacoes SET foto=?, titulo=?, postado_por=?, conteudo=?, data_postagem=?, link=?, breve=?, meta_title=?, meta_keywords=?, meta_description=?, foto1=?, url_amigavel=?, descricao_imagem=?, legenda_imagem=?, id_categoria=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $titulo);   
					$stm->bindValue(3, $postado_por);
